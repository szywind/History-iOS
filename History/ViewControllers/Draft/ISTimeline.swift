...
open class ISTimeline: UIScrollView {
    ...
    open var linePositionX: CGFloat = 0.0

    open var bubbleArrows:Bool = true
    
    fileprivate var sections:[(point:CGPoint, bubbleRect:CGRect, descriptionRect:CGRect?, titleLabel:UILabel, descriptionLabel:UILabel?, pointColor:CGColor, lineColor:CGColor, fill:Bool, isRight:Bool)] = []
    ...
    override open func draw(_ rect: CGRect) {
        ...
        for i in 0 ..< sections.count {
            ...
            drawBubble(sections[i].bubbleRect, backgroundColor: bubbleColor, textColor:titleColor, titleLabel: sections[i].titleLabel, isRight: sections[i].isRight)
            
            let descriptionLabel = sections[i].descriptionLabel
            if (descriptionLabel != nil) {
                drawDescription(sections[i].descriptionRect!, textColor: descriptionColor, descriptionLabel: sections[i].descriptionLabel!, isRight: sections[i].isRight)
            }
        }
    ...
    fileprivate func buildSections() {
        ...
        for i in 0 ..< points.count {
            ...
            let point = CGPoint(x: linePositionX, //self.bounds.origin.x + self.contentInset.left + lineWidth / 2 + linePositionX,
            ...
            var x0 = point.x + pointDiameter + lineWidth / 2 + offset
            var x1 = x0
            var x2 = x0
            if !points[i].isRight {
                x1 = 2 * linePositionX - x1 - titleWidth + 5
            }
            ...
            if descriptionLabel != nil {
                if !points[i].isRight {
                    x2 = 2 * linePositionX - x2 - calcWidth() + 5
                }
            ...
            sections.append((point, bubbleRect, descriptionRect, titleLabel, descriptionLabel, points[i].pointColor.cgColor, points[i].lineColor.cgColor, points[i].fill, points[i].isRight))
    ...
    fileprivate func calcWidth() -> CGFloat {
        return (self.bounds.width - (self.contentInset.left + self.contentInset.right) - pointDiameter - lineWidth - ISTimeline.gap * 1.5) / 2.0
    }
    ...
    // draw bubble
    fileprivate func drawBubble(_ rect:CGRect, backgroundColor:UIColor, textColor:UIColor, titleLabel:UILabel, isRight: Bool = true) {
        if isRight {
            drawBubbleRight(rect, backgroundColor: backgroundColor, textColor: textColor, titleLabel: titleLabel)
        } else {
            drawBubbleLeft(rect, backgroundColor: backgroundColor, textColor: textColor, titleLabel: titleLabel)
        }
    }
    
    fileprivate func drawBubbleRight(_ rect:CGRect, backgroundColor:UIColor, textColor:UIColor, titleLabel:UILabel) {
    ...
    fileprivate func drawBubbleLeft(_ rect:CGRect, backgroundColor:UIColor, textColor:UIColor, titleLabel:UILabel) {
        let path = UIBezierPath(roundedRect: rect, cornerRadius: bubbleRadius)
        
        if bubbleArrows {
            let startPoint = CGPoint(x: rect.origin.x + rect.size.width, y: rect.origin.y + rect.height / 2 - 8)
            path.move(to: startPoint)
            path.addLine(to: startPoint)
            path.addLine(to: CGPoint(x: rect.origin.x + rect.size.width + 8, y: rect.origin.y + rect.height / 2))
            path.addLine(to: CGPoint(x: rect.origin.x + rect.size.width, y: rect.origin.y + rect.height / 2 + 8))
        }
        ...
        let titleRect = CGRect(x: rect.origin.x + 5, y: rect.origin.y, width: rect.size.width - 15, height: rect.size.height - 1)
        titleLabel.textColor = textColor
        titleLabel.frame = titleRect
        self.addSubview(titleLabel)
    }
    ...
    // draw description
    fileprivate func drawDescription(_ rect:CGRect, textColor:UIColor, descriptionLabel:UILabel, isRight:Bool) {
        if isRight {
            drawDescriptionRight(rect, textColor: textColor, descriptionLabel: descriptionLabel)
        } else {
            drawDescriptionLeft(rect, textColor: textColor, descriptionLabel: descriptionLabel)
        }
    }
    
    fileprivate func drawDescriptionRight(_ rect:CGRect, textColor:UIColor, descriptionLabel:UILabel) {
        descriptionLabel.textColor = textColor
        descriptionLabel.frame = CGRect(x: rect.origin.x + 7, y: rect.origin.y, width: rect.width - 10, height: rect.height)
        self.addSubview(descriptionLabel)
    }
    
    fileprivate func drawDescriptionLeft(_ rect:CGRect, textColor:UIColor, descriptionLabel:UILabel) {
        descriptionLabel.textColor = textColor
        descriptionLabel.frame = CGRect(x: rect.origin.x + 3, y: rect.origin.y, width: rect.width - 10, height: rect.height)
        self.addSubview(descriptionLabel)
    }
